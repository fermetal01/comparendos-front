...
export class VehiculoComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.vehiculoService
      ...
      .subscribe(
        (res: HttpResponse<IVehiculo[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IVehiculo): string {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInVehiculos(): void {
    this.eventSubscriber = this.eventManager.subscribe('vehiculoListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IVehiculo[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
