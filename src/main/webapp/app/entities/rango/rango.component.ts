...
export class RangoComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.rangoService
      ...
      .subscribe(
        (res: HttpResponse<IRango[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IRango): string {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInRangos(): void {
    this.eventSubscriber = this.eventManager.subscribe('rangoListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IRango[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/rango'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
