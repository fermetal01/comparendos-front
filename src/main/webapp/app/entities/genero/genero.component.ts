...
export class GeneroComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.generoService
      ...
      .subscribe(
        (res: HttpResponse<IGenero[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IGenero): string {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInGeneros(): void {
    this.eventSubscriber = this.eventManager.subscribe('generoListModification', () => this.loadPage());
  }
