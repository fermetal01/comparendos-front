...
export class PersonaService {
  ...
  protected convertDateFromClient(persona: IPersona): IPersona {
    const copy: IPersona = Object.assign({}, persona, {
      fechaNacimiento:
        persona.fechaNacimiento && persona.fechaNacimiento.isValid() ? persona.fechaNacimiento.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((persona: IPersona) => {
        persona.fechaNacimiento = persona.fechaNacimiento ? moment(persona.fechaNacimiento) : undefined;
      });
    }
    return res;
  }
