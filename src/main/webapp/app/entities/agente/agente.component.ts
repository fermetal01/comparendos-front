...
export class AgenteComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.agenteService
      ...
      .subscribe(
        (res: HttpResponse<IAgente[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IAgente): string {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInAgentes(): void {
    this.eventSubscriber = this.eventManager.subscribe('agenteListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IAgente[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
